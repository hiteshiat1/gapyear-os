-- Idempotent catch-up script for gapyear-os / ALevels.io production Supabase project.
-- Safe to run in full even if some parts were already applied (every statement uses
-- "if not exists" / "drop policy if exists" / "on conflict do update" style guards).
--
-- Run this entire file once in the Supabase SQL editor. It applies, in order:
--   1. 20260818_profile_theme.sql        -> adds student_profiles.visual_tone
--      (this was the missing piece causing "Could not find the 'visual_tone' column
--       of 'student_profiles'" on POST /onboarding)
--   2. 20260818_future_map.sql           -> interests/universities/careers/future map tables
--      (skip if you're certain this already ran — it's harmless to re-run either way)
--
-- Already applied earlier and NOT included here (skip re-running unless you want to
-- double check they're idempotent-safe, which they are):
--   - 20260818_alevels_onboarding.sql
--   - 20260818_reference_onboarding.sql
--   - 20260819_reference_data_admin_write.sql

-- ============================================================
-- 1. 20260818_profile_theme.sql
-- ============================================================
