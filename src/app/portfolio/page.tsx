...
import { portfolioProjects, subjects } from "@/lib/data";
      ...
      <section className="border-b border-slate-200 bg-slate-50">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <div className="mt-10 max-w-3xl">
            ...
            <h1 className="mt-3 text-4xl font-semibold tracking-tight sm:text-5xl">Sachith</h1>
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-4 py-10 sm:px-6 lg:grid-cols-3 lg:px-8">
        {subjects.filter((subject) => subject.active).map((subject) => (
          <Card key={subject.id}>
            ...
            <p className="mt-2 text-sm text-slate-500">Target: {subject.targetGrade}</p>
            <div className="mt-4">
        ...
        <div className="mt-5 grid gap-6 lg:grid-cols-2">
          {portfolioProjects.map((project) => (
            <Card key={project.title}>
              ...
              <p className="mt-3 text-sm font-medium">{project.tech}</p>
            </Card>
          ))}
        </div>
      </section>
